  ...
  try {
    ...
    if (req.query.type === "saleinvoice") {
      ...
      const totalUnitMeasurement = singleSaleInvoice.saleInvoiceProduct.reduce(
        (acc, item) =>
          acc +
          Number(item.warehouseStock.unitMeasurement) * item.productQuantity,
        0
      );
      invoiceData = {
        status,
        ...
        <div class="body-table">
          <table>
            ...
            <tr>
              ...
              <td>${invoiceData.singleSaleInvoice.totalAmount}</td>
            </tr>
            <tr>
                ...
                <tbody>
                  ${invoiceData.singleSaleInvoice.saleInvoiceProduct
                    ...
                    <tr>
                      ...
                      <td>${item.warehouseStock.unitMeasurement} ${
                        item.warehouseStock.unitType
                      }</td>
                      <td>${item.productQuantity}</td>
                      <td>${item.productSalePrice}</td>
                      <td>${item.productQuantity * item.productSalePrice}</td>
                    </tr>
                    `;
                    })
                    .join("")}
